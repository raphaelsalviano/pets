...
import type { ViewStylePropTypes } from '../types/StylesTypes';

type Props = {
    +buttonComponent: any,
    +menuItems: Array<DropdownItemType>,
    +onClose?: (item: ?DropdownItemType) => mixed,
...
class DropdownMenu extends PureComponent<Props, State> {
    ...
    onMenuLayout = () => {
        ...
        const { width, height } = this.props.sizeCard;

        let heightMenu = height;
    ...
    renderItem = ({ item }: { item: DropdownItemType }) => {
        const dropdownItemProps = {
            item,
            disabled: false,
            // eslint-disable-next-line prettier/prettier
            onSelected: (itemSelected: DropdownItemType) => {
        ...
        return <DropdownItem {...dropdownItemProps} />;
    };

    renderItems = (menuItems: Array<DropdownItemType>) => {
        ...
        const flatlistProps = {
            numColumns: 1,
            data: menuItems,
            keyExtractor: (item: DropdownItemType) => (typeof item.value === 'number'
                    ? item.value.toString()
                    : item.value),
                ...
                style,
                {
                    ...Platform.select({
                        android: {
                            ...
                            borderColor: Platform.Version < 20 ? theme.variant === 'dark' ? 'rgba(255, 255, 255, .12)' : 'rgba(0, 0, 0, .25)' : 'transparent'
                        }
                    }),
                    backgroundColor:
                        theme.variant === 'dark'
